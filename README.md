...
- **懒建表不是"自动建表"，是"点开过那个功能才有那张表"**（P7c 删掉 Node 侧 9 个 `ensure*`
  与 41 处调用点的理由）：它在第一次用到时才建，所以一份没被点过的库就是缺那张表、缺那一列、
  缺那个索引；更糟的是双轨期**缺哪一块取决于第一个请求走的是哪栈**——Node 的懒迁移与 Java 的
  建库器各建各的，两边都绿，而对拍永远看不见，因为两边都假设对方已经把库建好了。
  删不能只删：先补 schema（三条只活在懒迁移里的 DDL），再反证结构齐全（闸门 16 §2 最后两行：
  拿**运行库**的每一条索引、每一条外键连级联规则反查回 Java 建的库），最后立一条静态不变量
...
#   · §0 先钉归属（静态扫描，不连库也不启进程）：lib / app / components / middleware 里不许出现
#     一条 DDL，连注释与模板串里藏着建表 SQL 都算红。判据不是「跑起来没报错」，而是
#     **表结构只有一个地方写着**：两处各建各的表时两边都绿，而「新库长什么样」
#     取决于第一个敲到接口的请求走的是哪栈。注释也扫，是因为藏在注释里的 DDL
#     会在下一次改代码时复活。
#   · §1–§5 测的是**进程**不是脚本：闸门自己 CREATE 三个临时库、真跑 mvn spring-boot:run、
#     等它应答——「脚本看着对」和「启动时真的会跑、跑完还能服务」之间隔着打包、
#     Bean 初始化顺序与幂等与否，这些都没法靠读代码确认。五个环节每一环都是拿**真库**比的：
...
#       外键连 DELETE_RULE 一起比：级联与 RESTRICT 是「同一条外键」的两种不同东西，
#       只比名字与两端的话，删同一篇文章会一侧成功、一侧 500 而结构比对全绿。
#       本节最后两行是 P7c 删懒迁移的正面依据：把**运行库**的每一条索引、每一条外键（含级联规则）
#       反查回 Java 建的库。前面那些行比的是 Java 与 schema.sql——同一份定义的两个执行者，
#       对不出「定义本身漏了什么」；而运行库是被 Node 那批 ensure* 一路改出来的，
#       它身上有、脚本给不出的东西，就是删掉懒迁移之后新库永远缺的那一块。
...
#   · 共 18 项。跑之前确认 server/target/classes 是最新编译产物（闸门自己比 mtime，不新就红）。
