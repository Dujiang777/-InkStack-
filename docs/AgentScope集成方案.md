...
> ⚠ **本方案已被取代（2026-09-24，P6c）**：智能体框架换成 **Spring AI**，进程从
> "Next → Python FastAPI/AgentScope" 收成 "Java 后端内置"，`agent-service/` 已随本仓库删除
> （原实现仍可在参考仓库里读到）。下面保留当初选 AgentScope 的理由，其中
> "模型走 OpenAI 兼容协议""检索直查 MySQL 全文索引"两条被新方案原样继承；
> 新方案的开关见 `DEPLOY.md` 第 6 节与 README 的闸门 15。
> 一处实质修正：本文档时期那版检索 SQL 缺"过审"与付费墙判定，会把未解锁的付费正文喂给模型，
> 新方案按 `lib/rag.ts` 的口径重做并补上了这两道。
